{
  ...
  plugins = {
    aac-codec = callPackage ./plugins/aac-codec.nix {};
    ffmpeg-encoder = callPackage ./plugins/ffmpeg-encoder.nix {};
  };
  ...
  asoundConfig = writeText "asoundrc" ''
    pcm.!default {
      type pulse
    }
    ctl.!default {
    ...
    stdenv.mkDerivation rec {
      ...
      src =
        ...
        rec {
          ...
          outputHash =
            if studioVariant
            then "sha256-q5VGp0kkno//nYtT82QDZDJG92uumAtomUK4B55795g="
            else "sha256-2pfJz71fS/oEmK3n4cESKb9EDYCeDBhbzGLgFpb+OLI=";
      ...
      in ''
        ...
        mkdir -p $out/{"Apple Immersive/Calibration",configs,DolbyVision,easyDCP,Extras,Fairlight,GPUCache,logs,Media,"Resolve Disk Database",.crashreport,.license,.LUT,IOPlugins}
      ...
      postInstall = lib.optionalString studioVariant ''
  ...
  buildFHSEnv {
    ...
    targetPkgs = pkgs:
      with pkgs;
        ...
        ++ lib.concatLists (map (drv: drv.runtimeDependencies or []) [plugins.aac-codec plugins.ffmpeg-encoder]);
    ...
    extraBwrapArgs =
      [
        ...
        "--ro-bind ${asoundConfig} /etc/asound.conf"
      ]
    ...
    runScript = "${bash}/bin/bash ${writeText "davinci-wrapper" ''
      ...
      export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:/usr/lib:/usr/lib32:${davinci}/libs:${ffmpeg}/lib
      if [ $# -gt 0 ]; then
        exec "$@"
      else
